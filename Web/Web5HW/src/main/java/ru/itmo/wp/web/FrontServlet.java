...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        for (Class<?> clazz = pageClass; beforeMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            List<Method> methodList = Arrays.stream(clazz.getDeclaredMethods()).filter((Method m) -> m.getName().equals("before")).collect(Collectors.toList());
            if (methodList.size() > 0) {
                beforeMethod = methodList.get(0);
            } else {
                beforeMethod = null;
            }
        }
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            List<Method> methodList = Arrays.stream(clazz.getDeclaredMethods()).filter((Method m) -> m.getName().equals(route.getAction())).collect(Collectors.toList());
            if (methodList.size() > 0) {
        ...
        for (Class<?> clazz = pageClass; afterMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            List<Method> methodList = Arrays.stream(clazz.getDeclaredMethods()).filter((Method m) -> m.getName().equals("after")).collect(Collectors.toList());
            if (methodList.size() > 0) {
                afterMethod = methodList.get(0);
            } else {
                afterMethod = null;
            }
        }
        ...
        try {
            ...
            Object[] parameters = new Object[typesOfParameters.length];
            for (int i = 0; i < parameters.length; i++){
                if (typesOfParameters[i] == Map.class){
                    parameters[i] = view;
                } else if (typesOfParameters[i] == HttpServletRequest.class){
